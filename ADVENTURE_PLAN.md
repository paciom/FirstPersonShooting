...
Each beat also carries a **`shot`** line: one sentence describing what the
30-second clip shows. Text mode ignores it. It is the entire hand-off to video
mode, written while the beat is being written, when the picture is still in
mind.
...
- [ ] Video mode: one 30-second clip per beat, driven by each beat's `shot`
      line, with the reader's graph walk kept exactly as it is.
