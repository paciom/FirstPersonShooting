...
public static class MenuIconRigs
{
        ...
        bool preferWalker = false)
    {
        ...
        // The fighter prefab's rest pose is a side-on fighting stance, which is
        // right for a bout and wrong for a line-up facing the camera; the
        // walker's rest pose is the neutral front stance the select screen and
        // PreviewCaptures both show.
        var fighterPrefab = preferWalker
            ? null
            : Resources.Load<GameObject>($"Brawl/{robot}-fighter");
    ...
    /// </summary>
    /// <summary>
    /// The title screen's hero shot: five roster robots in a shallow V, facing
    /// the camera from a low angle. Deliberately floorless — this render is
    /// cut out and composited onto painted sky, so anything under their feet
    /// would come along as a slab hanging in mid-air.
    ///
    /// Public because MenuArtForge renders it once to a still; nothing builds
    /// it at runtime.
    /// </summary>
    public static Camera BuildHeroLineup(Transform rig, RobotRoster roster, RenderTexture rt,
        float yaw, bool preferWalker)
    {
        // Centre robot nearest and tallest, wings stepping back and down, so
        // the group reads as a wedge rather than as a row.
        (string robot, int fallback, float x, float z, float height, int team)[] line =
        {
            ("titan", 0, 0f, 0.1f, 1.80f, 0),
            ("hawk", 1, -1.02f, 0.52f, 1.62f, 1),
            ("panther", 2, 1.02f, 0.52f, 1.62f, 0),
            ("samurai", 3, -1.94f, 1.02f, 1.48f, 1),
            ("ranger", 4, 1.94f, 1.02f, 1.48f, 0),
        };
        foreach (var (robot, fallback, x, z, height, team) in line)
        {
            // Which yaw points a fighter at the camera is not obvious: these
            // rigs prefer the re-rigged Brawl fighter prefab, whose rest
            // orientation is not the walker's. MenuArtForge renders the whole
            // line-up at four yaws and the compositor picks — hence the
            // parameter rather than a constant.
            FrozenRobot(rig, Cast(roster, robot, fallback), team, null, 0f,
                new Vector3(x, 0f, z), yaw, height, preferWalker);
        }
